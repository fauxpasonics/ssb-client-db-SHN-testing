...
CREATE PROCEDURE
[rpt].[Email_Summary_v2]

AS


SELECT DISTINCT dc.SSB_CRMSYSTEM_CONTACT_ID
INTO #Unsubscribes
...
SELECT DISTINCT dc.SSB_CRMSYSTEM_CONTACT_ID, ecr.List_Name
INTO #Deliveries
FROM ods.VerticalResponse_EmailCampaign_Recipients ecr (NOLOCK)
JOIN dbo.vwDimCustomer_ModAcctId dc (NOLOCK)
	ON ecr.Email_Address = dc.EmailPrimary
	AND dc.SourceSystem = 'VerticalResponse'
LEFT JOIN #Unsubscribes u ON u.SSB_CRMSYSTEM_CONTACT_ID = dc.SSB_CRMSYSTEM_CONTACT_ID
WHERE ec.MailDate < ISNULL(u.UnsubDate, GETDATE())


SELECT dc.SSB_CRMSYSTEM_CONTACT_ID, COUNT(*) OpenCount, MIN(eca.Response_Date) MinOpenDate, MAX(eca.Response_Date) MaxOpenDate
INTO #all_open
FROM ods.VerticalResponse_EmailCampaign_Recipients eca (NOLOCK)
JOIN dbo.vwDimCustomer_ModAcctId dc (NOLOCK)
ON eca.Email_Address = dc.EmailPrimary
AND dc.SourceSystem = 'VerticalResponse'
...
INTO #no_open
FROM #Deliveries d WITH (NOLOCK)
LEFT JOIN #all_open o WITH (NOLOCK) ON d.SSB_CRMSYSTEM_CONTACT_ID = o.SSB_CRMSYSTEM_CONTACT_ID
WHERE o.SSB_CRMSYSTEM_CONTACT_ID IS NULL

--DROP TABLE #opens

SELECT DISTINCT SSB_CRMSYSTEM_CONTACT_ID, MAX(MaxOpenDate) AS MaxOpenDate
INTO #opens
FROM #all_open WITH (NOLOCK)
WHERE OpenCount > 0
