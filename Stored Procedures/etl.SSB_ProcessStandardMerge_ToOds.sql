...
SET
	 ...
	 ( SELECT STUFF ((
                    SELECT ', [' + name + ']'
                    FROM sys.columns
                    WHERE object_id = OBJECT_ID(@Source) and name not in ('ETL__ID','ETL__CreatedDate')		
					ORDER BY column_id		
                    FOR XML PATH('')), 1, 1, '') 
	 )
...
EXEC  etl.SSB_HashFieldSyntax @Source, 'ETL__ID,ETL__CreatedDate'
...
DECLARE @OrderByString NVARCHAR(255) = (SELECT CASE WHEN ISNULL(@OrderBy,'') = '' THEN 'ETL__ID' ELSE @OrderBy END);
	...
	AND COLUMN_NAME NOT IN ('ETL__ID','ETL__CreatedDate')
      ...
      ' +
          STUFF ((
                    SELECT ',myTarget.[' + name + '] = mySource.[' + name + ']' + CHAR(10) + '     '
						...
						AND name NOT IN ('ETL__ID','ETL__CreatedDate','ETL__UpdatedDate','ETL__IsDeleted','ETL__DeletedDate')
						ORDER BY column_id
                    FOR XML PATH('')), 1, 1, '')  +
