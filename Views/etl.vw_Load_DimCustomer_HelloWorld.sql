...
*****/
	...
	CREATE VIEW [etl].[vw_Load_DimCustomer_HelloWorld] AS 

	WITH MaxCreated (phone, signupstamp)
AS (
	SELECT phone, MAX(signupstamp) signupstamp
	FROM ods.HelloWorld_Subscribers (NOLOCK)
	GROUP BY phone
)
, MinCreated (phone, signupstamp)
AS (
	SELECT phone, MIN(signupstamp) signupstamp
	FROM ods.HelloWorld_Subscribers (NOLOCK)
	GROUP BY phone
)
...
SELECT  DB_NAME() AS [SourceDB]
      , 'HelloWorld' AS [SourceSystem]
      , 3 AS [SourceSystemPriority]


/*Standard Attributes*/
      , s.phone AS [SSID]
      , NULL AS [CustomerType]
      , CASE WHEN u.unsubscribe_stamp IS NOT NULL AND u.unsubscribe_stamp < GETDATE() THEN 'Unsubscribed'
			ELSE 'Subscribed'
...
/*AddressPrimary*/
      ...
      , s.country AS [AddressPrimaryCountry]
...
/*Phone*/
      , CAST(s.Phone AS NVARCHAR(25)) AS [PhonePrimary]
      , NULL AS [PhoneHome]
      , CAST(s.Phone AS NVARCHAR(25)) AS [PhoneCell]
...
/*Extended Attributes*/
      , s.carrier_name AS [ExtAttribute1] --nvarchar(100)
      , u.unsubscribe_type AS [ExtAttribute2]
...
/*Source Created and Updated*/
      , NULL AS [SSCreatedBy]
      , NULL AS [SSUpdatedBy]
      , CAST(mincreate.signupstamp AS DATETIME) AS [SSCreatedDate]
      , CAST(COALESCE(u.unsubscribe_stamp, maxcreate.signupstamp) AS DATETIME) AS [SSUpdatedDate]
...
--		select top 100 *
		FROM ods.HelloWorld_Subscribers s (NOLOCK)
		JOIN MaxCreated maxcreate ON s.phone = maxcreate.phone AND s.signupstamp = maxcreate.signupstamp
		JOIN MinCreated mincreate ON s.phone = mincreate.phone
		LEFT JOIN ods.HelloWorld_Unsubscribes u (NOLOCK)
			ON s.subscriberid = u.subscriberid
			AND s.listid = u.listid
			AND s.phone = u.phone
		WHERE   1 = 1
		AND s.SignUpStamp > DATEADD(DAY,-3,GETDATE())
