...
SELECT * FROM 
	( SELECT    ssbid.SSB_CRMSYSTEM_CONTACT_ID
              , fts.ETL__SSID_AV_customer_id AS Customer_ID
			  , DimCustomer.AccountId AS Customer_Number
		...
		--, DimEvent.MinorCategoryTM
              ...
              , DimPriceType.PriceTypeCode AS Price_Type_Code
              , DimPriceType.PriceTypeDesc AS Price_Type_Description
              , DimPriceType.PriceTypeClass AS Price_Type_Class
              , LEFT(DimPriceType.PriceTypeCode, 1) AS PC1
              , SUBSTRING(DimPriceType.PriceTypeCode, 2, 1) AS PC2
              , SUBSTRING(DimPriceType.PriceTypeCode, 3, 1) AS PC3
              , SUBSTRING(DimPriceType.PriceTypeCode, 4, 1) AS PC4
			  , DimPriceType.PriceTypeCode AS Price_Type
		...
		--, DimSaleCode.SalesCodeClass
			  ...
			  , fts.CreatedBy Created_By
      FROM      dbo.FactTicketSales_V2 fts WITH ( NOLOCK )
                INNER JOIN rpt.vw_DimPriceType DimPriceType WITH ( NOLOCK ) ON DimPriceType.DimPriceTypeID = fts.DimPriceTypeId
				...
				INNER JOIN rpt.vw_DimTicketCustomer DimTicketCustomer WITH (NOLOCK) ON DimTicketCustomer.DimTicketCustomerId = fts.DimTicketCustomerId
                INNER JOIN dbo.DimCustomer DimCustomer WITH ( NOLOCK )
					ON DimCustomer.SourceSystem = DimTicketCustomer.ETL__SourceSystem
					AND DimTicketCustomer.CustomerID = DimCustomer.SSID
					AND Dimcustomer.CustomerStatus = 'Primary'
                ...
                INNER JOIN rpt.vw_DimSeat DimSeat WITH ( NOLOCK ) ON DimSeat.DimSeatId = fts.DimSeatId_Start
                INNER JOIN rpt.vw_DimItem DimItem WITH ( NOLOCK ) ON DimItem.DimItemId = fts.DimItemId
                INNER JOIN rpt.vw_DimEventHeader DimEventHeader WITH ( NOLOCK ) ON DimEvent.AV_series_id = DimEventHeader.ETL__SSID_AV_series_id
				...
				INNER JOIN rpt.vw_DimTeam DimTeam  WITH ( NOLOCK ) ON DimEventHeader.DimTeamId_Opponent = DimTeam.DimTeamId
